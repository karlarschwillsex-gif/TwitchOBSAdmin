...
const COUNTDOWN_FILE     = path.join(ROOT, 'data', 'countdown.json');
const COUNTDOWN_HTML_OUT = '/home/fynn/Dokumente/OBS Text/countdown.html';
...
app.post('/api/admin/cam-filter/trigger', async (req, res) => {
  ...
  try {
    ...
    setTimeout(async () => {
      try { await setFilterState(filterName, false); }
      catch (e) { console.error('[server] Filter deaktivieren Fehler:', e.message); }
    }, durationMs);
    res.json({ ok: true });
  } catch (e) {
...
/* ============================================================
   CAM-REWARDS API
   ============================================================ */
app.get('/api/admin/cam-rewards', (req, res) => res.json(safeReadJson(CAM_REWARDS_FILE, [])));
app.post('/api/admin/cam-rewards', (req, res) => {
  res.json({ ok: safeWriteJson(CAM_REWARDS_FILE, Array.isArray(req.body) ? req.body : []) });
});
app.post('/api/admin/cam-rewards/create', async (req, res) => {
  ...
  try {
    ...
    const reward = await twitchRewards.createReward({ title: rewardName, cost: Number(cost) || 300, is_enabled: true, is_user_input_required: false });
    const rewards = safeReadJson(CAM_REWARDS_FILE, []);
    rewards.push({ rewardName, filterName, duration: Number(duration) || 10, cost: Number(cost) || 300, rewardId: reward?.id || null });
...
app.post('/api/admin/cam-rewards/update-twitch', async (req, res) => {
  ...
  try { const tr = require('./backend/twitchRewards.js'); await tr.updateReward(rewardId, { cost: Number(cost) }); res.json({ ok: true }); }
  catch (e) { res.json({ ok: false, error: e.message }); }
});
app.post('/api/admin/cam-rewards/delete-twitch', async (req, res) => {
  const { rewardId } = req.body || {};
  if (!rewardId) return res.status(400).json({ ok: false, error: 'rewardId fehlt' });
  try { const tr = require('./backend/twitchRewards.js'); await tr.deleteReward(rewardId); res.json({ ok: true }); }
  catch (e) { res.json({ ok: false, error: e.message }); }
});
...
app.get('/api/admin/sound-files', (req, res) => {
  try {
    const files = fs.readdirSync(SOUNDS_DIR).filter(f => /\.(mp3|wav|ogg|m4a)$/i.test(f)).sort();
    res.json({ files });
...
/* ============================================================
   SOUND-REWARDS
   ============================================================ */
app.get('/api/admin/sound-rewards',  (req, res) => res.json(safeReadJson(SOUND_REWARDS_FILE, [])));
app.post('/api/admin/sound-rewards', (req, res) => res.json({ ok: safeWriteJson(SOUND_REWARDS_FILE, Array.isArray(req.body) ? req.body : []) }));
app.post('/api/admin/sound-rewards/create', async (req, res) => {
  ...
  try {
    const tr = require('./backend/twitchRewards.js');
    const reward = await tr.createReward({ title: `${rewardName} [Sound]`, cost: Number(cost) || 100, is_enabled: true, is_user_input_required: false });
...
/* ============================================================
   COMMANDS API
   ============================================================ */
app.get('/api/admin/commands',  (req, res) => res.json(safeReadJson(COMMANDS_FILE, [])));
app.post('/api/admin/commands', (req, res) => res.json({ ok: safeWriteJson(COMMANDS_FILE, Array.isArray(req.body) ? req.body : []) }));
...
/* ============================================================
   COUNTDOWN API
   ============================================================ */
app.get('/api/admin/countdown', (req, res) => res.json(safeReadJson(COUNTDOWN_FILE, {})));
app.post('/api/admin/countdown', (req, res) => {
  const { settings, html } = req.body || {};
  safeWriteJson(COUNTDOWN_FILE, settings || {});
  try {
    const dir = path.dirname(COUNTDOWN_HTML_OUT);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(COUNTDOWN_HTML_OUT, html || '', 'utf8');
    res.json({ ok: true });
  } catch(e) {
    console.error('[server] Countdown speichern Fehler:', e.message);
    res.json({ ok: false, error: e.message });
  }
});
